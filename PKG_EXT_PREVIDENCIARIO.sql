CREATE OR REPLACE PACKAGE OWN_FUNCESP.PKG_EXT_PREVIDENCIARIO IS
 ...
 -- VARIAVEIS
  ...
  --G_NAME         VARCHAR2(255)         := 'GeracaoExtratoCorreio_044_031_012021.txt'; 
  G_READ         CHAR(1)               := 'R';
  G_SIZE         NUMBER                := 32767;
...
/*            DBMS_OUTPUT.PUT_LINE(REC_CARGA.TPO_DADO ||' '||
      ...
      EXCEPTION
        ...
        WHEN No_data_found THEN
            UTL_FILE.FCLOSE(G_ARQ);
            DBMS_OUTPUT.PUT_LINE('Fim de Arquivo');
        WHEN OTHERS THEN
            UTL_FILE.FCLOSE(G_ARQ);
            DBMS_OUTPUT.PUT_LINE('CODIGO ERRO: '||SQLCODE|| ' - '||'MSG: '||SQLERRM);
            DBMS_OUTPUT.PUT_LINE('LINHA: '||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
    ...
    BEGIN             
      ...
      FOR RG_TRATA_DADOS IN C_TRATA_DADOS 
         LOOP
           -- INSERT 
           --DBMS_OUTPUT.PUT_LINE(RG_TRATA_DADOS.COD_EMPRS);
      ...
      END IF; 
        ...
        -- LIMPA A TABELA DE ORIGEM APOS EFETUAR A CARGA NA TABELA DE DESTINO: ATT.FC_PRE_TBL_BASE_EXTRAT_CTB
